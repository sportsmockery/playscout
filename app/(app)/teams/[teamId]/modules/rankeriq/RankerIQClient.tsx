...
export default function RankerIQClient({
  ...
  const [ownResult, setResult] = useState<RankerResult | null>(null);
  ...
  const adopted = latestFor('RANKERIQ', teamId);
  const fromShell = adopted?.status === 'complete' ? adopted : null;
  const result = ownResult ?? ((fromShell?.result as RankerResult | undefined) ?? null);
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      setResult(run.result as RankerResult);
