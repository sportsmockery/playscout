...
import { Zap, ChevronDown, AlertCircle, Layers } from 'lucide-react';
...
export default function QBIQClient({ teamId, teamName, ageGroup, qbs, videos, folders, pastAnalyses, initialVideoIds }: Props) {
  ...
  const [ownResult, setResult] = useState<AnalysisResult | null>(null);
  ...
  const adopted = latestFor('QBIQ', teamId);
  const fromShell = adopted?.status === 'complete' ? adopted : null;
  const result = ownResult ?? ((fromShell?.result as AnalysisResult | undefined) ?? null);
    ...
    try {
      if (!runsInline) {
        const res = await queueAnalysisBatch({
          ...
          title: batchTitle('QBIQ', selectedVideoIds.length),
          context: buildContext(),
        });
        setQueued(`${res.queued} clip${res.queued === 1 ? '' : 's'} queued — results appear below as they finish.`);
      ...
      setResult(run.result as AnalysisResult);
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-1 space-y-5">
          ...
          <div className="space-y-4">
            ...
            >
              {loading ? (
                ...
                <>
                  <Layers size={16} />
                  Queue QBIQ on {selectedVideoIds.length} clip{selectedVideoIds.length === 1 ? '' : 's'}
                </>
              )}
            </button>
        ...
        <AnalysisQueue teamId={teamId} moduleKey="QBIQ" refreshKey={queueVersion} />
