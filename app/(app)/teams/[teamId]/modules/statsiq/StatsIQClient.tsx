...
export default function StatsIQClient({
  ...
  const [ownResult, setResult] = useState<StatsResult | null>(null);
  ...
  const adopted = latestFor('STATSIQ', teamId);
  const fromShell = adopted?.status === 'complete' ? adopted : null;
  const result = ownResult ?? ((fromShell?.result as StatsResult | undefined) ?? null);
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      if (run.status === 'failed') throw new Error(run.error || 'Charting failed');
      setResult(run.result as StatsResult);
              ...
              <div>
                ...
                <p className="text-[11px] text-[var(--brand-muted)] mt-1">
                  {jerseyAmbiguous && jerseyChoice === 'unknown'
                    ? 'Pick the colour your team is actually wearing in this film — the wrong one puts the other team\u2019s players on your sheet.'
